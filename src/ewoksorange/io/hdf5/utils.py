...
FileType = Union[h5py.File, commonh5.File]
"""An HDF5 file, or one of the objects standing in for one."""

GroupType = Union[h5py.Group, commonh5.Group]
...
class FileFactory(Protocol):
    """Opens an HDF5 file by name.

    :class:`h5py.File` and the access classes of :mod:`ewoksorange.io.hdf5`
    are such factories, the keyword arguments being what each of them takes
    besides the file name.
    """

    def __call__(self, name: str, /, **file_arguments: Any) -> FileType: ...
