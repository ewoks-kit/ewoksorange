...
from .model import Hdf5TreeModel
...
class Hdf5TreeViewer(qt.QWidget):
    ...
    The new file is `None` when it could not be read: the tree then shows it as
    a broken item and :attr:`sigFileFailed` says why.
    """
    ...
    Reading is for showing data, so a failure is reported instead of raised.
    Connect to this to tell the user that what is shown is not everything.
    """
    ...
    def __createSortedModel(self, treeView: Hdf5TreeView) -> NexusSortFilterProxyModel:
        ...
        treeModel.sigFileFailed.connect(self.sigFileFailed)
    ...
    def __synchronizeH5pyObject(
        ...
        SILX DIFFERENCE with ``silx.app.view.Viewer.__synchronizeH5pyObject``,
        which always reloads in a worker thread and leaves the old file open:
        reloading happens in this thread unless the model asks for a worker,
        and the old file is closed once the new one is in the tree. Like silx,
        :meth:`silx.gui.hdf5.Hdf5TreeModel.synchronizeH5pyObject` is not used,
        because it reopens the root by its file name instead of the path it
        was opened with.
        """
    ...
    def __h5FileLoaded(self, loadedH5: FileType, filename: str = "") -> None:
        if loadedH5 is None:
            # A file which could not be read: it is in the tree as a broken
            # item and reported through sigFileFailed, but there is nothing to
            # browse and nothing to close.
            return
    ...
    def __h5FileSynchronized(
        self, removedH5: FileType, loadedH5: Optional[FileType]
    ) -> None:
        if removedH5 in self._h5files:
            self._h5files.remove(removedH5)
        if loadedH5 is not None:
            self._h5files.append(loadedH5)
    ...
    def updateFile(self, filename: str) -> None:
        ...
        if not self.__treeModel.exists(filename):
            return
        h5file = self.__getFileObject(filename)
        # Showing data must not make the caller fail: a file which cannot be
        # read is reported through sigFileFailed instead.
        try:
            if h5file is None:
                if self.__treeModel.backgroundLoading:
            ...
            _logger.warning("Cannot read '%s'", filename, exc_info=True)
            self.sigFileFailed.emit(filename, e)
