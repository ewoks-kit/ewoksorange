...
"""The HDF5 access class to use for each `--access` value."""

_ACCESS_HELP = {
    "static": "nobody writes it",
    "live": "another process may write it",
    "owned": "a task of this process may write it",
}
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    model = Hdf5TreeModel(ACCESS_CLASSES[args.access])
...
def _accessHelp() -> str:
    """Return one line per `--access` value, naming the class it uses."""
    width = max(len(name) for name in ACCESS_CLASSES)
    lines = []
    for name, fileClass in ACCESS_CLASSES.items():
        lines.append(f"{name:<{width}}  {fileClass.__name__}")
